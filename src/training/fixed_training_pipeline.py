#!/usr/bin/env python3
"""
Fixed Training Pipeline for 15 Government Exams
Fixed compatibility issue with Transformers library's compute_loss signature
"""
...
class FixedExamClassifier(nn.Module):
    """Custom multi-task classifier for government exams with fixed compatibility"""
    ...
    def __init__(self, model_name: str, num_subjects: int, num_topics: int, num_difficulties: int, dropout_rate: float = 0.3):
        ...
        self.difficulty_classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout_rate),
            nn.Linear(hidden_size // 2, num_difficulties)
        )
        
    def forward(self, input_ids=None, attention_mask=None, subject_labels=None, topic_labels=None, difficulty_labels=None, **kwargs):
...
class FixedExamDataset:
    """Custom dataset for exam questions with fixed compatibility"""
...
class FixedExamTrainer(Trainer):
    """Fixed trainer with proper compute_loss signature compatibility"""
    
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        """
        Fixed compute_loss method that accepts num_items_in_batch and other kwargs
        """
        # Extract labels
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None, **kwargs):
        """
        Fixed prediction_step with proper kwargs handling
        """
        with torch.no_grad():
            # Extract labels
            subject_labels = inputs.pop("subject_labels")
            ...
            return (
                loss,
                torch.stack([subject_preds, topic_preds, difficulty_preds], dim=1),
                torch.stack([subject_labels, topic_labels, difficulty_labels], dim=1)
            )

class FixedTrainingPipeline:
    """Fixed training pipeline with Transformers compatibility"""
    
    def __init__(self, base_dir="/workspace"):
        self.base_dir = Path(base_dir)
        self.output_dir = self.base_dir / "fixed_training_outputs"
    ...
    def train_fixed_model(self) -> Dict[str, Any]:
        """Train model with fixed compatibility"""
        ...
        # Create datasets
        train_dataset = FixedExamDataset(splits["train"], tokenizer, subject_encoder, topic_encoder, difficulty_encoder)
        val_dataset = FixedExamDataset(splits["validation"], tokenizer, subject_encoder, topic_encoder, difficulty_encoder)
        
        # Initialize model
        model = FixedExamClassifier(
        ...
        logger.info("Starting model training with fixed compatibility...")
        
        # Train the model
        trainer.train()
        
        # Save the trained model
        model_save_path = self.output_dir / "fixed_exam_model"
        ...
        # Save training data
        with open(self.output_dir / "fixed_training_data.json", 'w') as f:
            json.dump(splits, f, indent=2)
        
        logger.info(f"✅ Fixed model training completed successfully!")
        logger.info(f"Model saved to: {model_save_path}")
        logger.info(f"Configuration saved to: {self.output_dir / 'model_config.json'}")
        
        # Run evaluation
        results = self.evaluate_fixed_model(trainer, splits["test"])
    ...
    def evaluate_fixed_model(self, trainer, test_questions: List[Dict]) -> Dict[str, Any]:
        """Evaluate the trained model on test data"""
        
        logger.info("📊 Evaluating fixed model on test data...")
        ...
        # Create test dataset
        test_dataset = FixedExamDataset(test_questions, tokenizer, subject_encoder, topic_encoder, difficulty_encoder)
        
        # Run predictions
        predictions = trainer.predict(test_dataset)
        
        # Extract predictions and true labels
        pred_labels = predictions.predictions  # Shape: [batch_size, 3]
        true_labels = predictions.label_ids    # Shape: [batch_size, 3]
        
        # Calculate accuracies
        subject_preds = pred_labels[:, 0]
        topic_preds = pred_labels[:, 1]
        difficulty_preds = pred_labels[:, 2]
        
        subject_true = true_labels[:, 0]
        topic_true = true_labels[:, 1]
        difficulty_true = true_labels[:, 2]
        
        subject_accuracy = accuracy_score(subject_true, subject_preds)
        topic_accuracy = accuracy_score(topic_true, topic_preds)
        difficulty_accuracy = accuracy_score(difficulty_true, difficulty_preds)
        
        overall_accuracy = np.mean([subject_accuracy, topic_accuracy, difficulty_accuracy])
        
        # Detailed classification reports
        subject_report = classification_report(
            subject_true, subject_preds,
            target_names=config["subject_classes"],
            output_dict=True
        )
        
        topic_report = classification_report(
            topic_true, topic_preds,
            target_names=config["topic_classes"],
            output_dict=True
        )
        
        difficulty_report = classification_report(
            difficulty_true, difficulty_preds,
            target_names=config["difficulty_classes"],
            output_dict=True
        )
...
def main():
    ...
    logger.info("🚀 Starting Fixed Government Exam AI Training Pipeline")
    logger.info("=" * 60)
    ...
    # Train model with fixed compatibility
    results = pipeline.train_fixed_model()
